...
<div class="space-y-6 animate-fade-in">
    <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm p-4 sm:p-6 border border-gray-100 dark:border-gray-700">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
            <div class="mb-4 sm:mb-0">
                <h1 class="text-2xl sm:text-3xl font-bold font-display text-gray-900 dark:text-white mb-2">Transações</h1>
                <p class="text-sm sm:text-base text-gray-600 dark:text-gray-400">Gerencie todas as suas transações financeiras</p>
            </div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 p-4 sm:p-6">
        <h2 class="text-lg font-semibold font-display text-gray-900 dark:text-white mb-4 flex items-center">
            <i class="fa-solid fa-filter w-5 h-5 mr-2 text-[1.25rem] leading-none text-primary dark:text-blue-400" aria-hidden="true"></i>
            Filtros
        </h2>
        <form method="GET" action="<?= base_url('/transactions') ?>"
            ...
            <div>
                ...
                <select id="type" name="type"
                    class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-primary focus:border-primary transition-colors duration-200">
            ...
            <div>
                <label for="orderBy" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Ordenar por</label>
            ...
            <div class="flex flex-col sm:flex-row items-end space-y-2 sm:space-y-0 sm:space-x-2">
                ...
                <a href="<?= base_url('/transactions') ?>"
                    class="flex-1 sm:flex-initial px-4 py-2 bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 font-medium rounded-lg hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors duration-200 text-center">
                    Limpar
                </a>
            </div>
        </form>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden">
        ...
        <?php else: ?>
                ...
                <?php foreach ($transactions as $transaction): ?>
                    <div class="p-4 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors duration-150">
                        <div class="flex items-start justify-between mb-3">
                            <div class="flex items-center space-x-3 flex-1 min-w-0">
                                <div class="w-10 h-10 bg-blue-100 dark:bg-blue-900/30 rounded-full flex items-center justify-center flex-shrink-0">
                                    <span
                                        class="text-primary dark:text-blue-400 text-xs font-semibold"><?= strtoupper(substr($transaction['name'], 0, 1)) ?></span>
                                </div>
                            ...
                            <span
                                class="px-2.5 py-1 text-xs font-semibold rounded-full flex-shrink-0 ml-2 <?= $transaction['type'] === 'income' ? 'bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-300' : 'bg-red-100 dark:bg-red-900/30 text-red-800 dark:text-red-300' ?>">
                        ...
                        <div class="flex items-center justify-between mb-3">
                            <div>
                                <p
                                    class="text-lg font-bold font-display <?= $transaction['type'] === 'income' ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400' ?>">
                                    <?= $transaction['type'] === 'income' ? '+' : '-' ?> R$
                                    <?= number_format($transaction['amount'], 2, ',', '.') ?>
                                </p>
                                <p class="text-xs text-gray-500 dark:text-gray-400 mt-1"><?= date('d/m/Y', strtotime($transaction['date'])) ?></p>
                            </div>
                        </div>
                        <?php if ($transaction['description']): ?>
                            <p class="text-xs text-gray-500 dark:text-gray-400 mb-3 line-clamp-2"><?= esc($transaction['description']) ?></p>
                        <?php endif; ?>
                        <div class="flex items-center space-x-4 pt-3 border-t border-gray-100 dark:border-gray-700">
                            <a href="<?= base_url('/transactions/' . $transaction['id'] . '/edit') ?>"
                                class="flex-1 text-center px-3 py-2 text-sm text-primary dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300 hover:bg-indigo-50 dark:hover:bg-indigo-900/30 rounded-lg transition-colors duration-200">
                                Editar
                            </a>
                            <a href="<?= base_url('/transactions/' . $transaction['id'] . '/delete') ?>"
                                class="flex-1 text-center px-3 py-2 text-sm text-red-600 dark:text-red-400 hover:text-red-700 dark:hover:text-red-300 hover:bg-red-50 dark:hover:bg-red-900/30 rounded-lg transition-colors duration-200"
                            ...
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors duration-150">
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center">
                                        ...
                                        <span class="text-sm font-medium text-gray-900 dark:text-white"><?= esc($transaction['name']) ?></span>
                                    </div>
                                </td>
                                <td class="px-6 py-4">
                                    <span class="text-sm text-gray-900 dark:text-white font-medium"><?= esc($transaction['title']) ?></span>
                                </td>
                                <td class="px-6 py-4">
                                    <span class="text-sm text-gray-500 dark:text-gray-400"><?= esc($transaction['description'] ?? '-') ?></span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span
                                        class="px-3 py-1 inline-flex text-xs font-semibold rounded-full <?= $transaction['type'] === 'income' ? 'bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-300' : 'bg-red-100 dark:bg-red-900/30 text-red-800 dark:text-red-300' ?>">
                                        <?= $transaction['type'] === 'income' ? 'Entrada' : 'Saída' ?>
                                    </span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <div class="flex items-center justify-end space-x-3">
                                        <a href="<?= base_url('/transactions/' . $transaction['id'] . '/edit') ?>"
                                            class="text-primary dark:text-blue-400 hover:text-blue-900 dark:hover:text-blue-300 transition-colors duration-200 flex items-center">
                                            <i class="fa-solid fa-pen-to-square mr-1 text-[1rem] leading-none"
                                                aria-hidden="true"></i>
                                            Editar
                                        </a>
                                        <a href="<?= base_url('/transactions/' . $transaction['id'] . '/delete') ?>"
                                            class="text-red-600 dark:text-red-400 hover:text-red-900 dark:hover:text-red-300 transition-colors duration-200 flex items-center"
                                            onclick="return confirm('Tem certeza que deseja deletar esta transação?')">
            ...
            <?php if ($pagination && $pagination['totalPages'] > 1): ?>
                <div class="bg-gray-50 dark:bg-gray-800 px-4 sm:px-6 py-4 border-t border-gray-200 dark:border-gray-700">
                        ...
                        <nav class="flex items-center space-x-2 flex-wrap justify-center">
                            <?php if ($pagination['page'] > 1): ?>
                                <a href="?page=<?= $pagination['page'] - 1 ?>"
                                    class="px-3 sm:px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-sm font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-600 transition-colors duration-200">
                                    Anterior
                                </a>
                            <?php endif; ?>
